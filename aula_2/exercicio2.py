...
class ModeloBigramas:
    ...
    def verificar_tokenizador(self):
        ...
        if not any(os.path.exists(os.path.join(caminho, 'tokenizers/punkt/portuguese.pickle')) for caminho in nltk.data.path):
    ...
    def limpar_texto(self, texto):
        ...
        regex = [
            ['', r'\{.*\}'], [' ', r'<(\/|\\)?.+?>'], [' ', r'(http|https)://[^\s]+'],
            [r'\1"', r"(?u)(^|\W)[‘’′`']"], [r'"\1', r"(?u)[‘’`′'](\W|$)"], ['"', r'(?u)[‘’`′“”]'],
            ['"', r'(\"\")'], ['"', r'(\'\')'], ['.', r'(?<!\.)\.\.(?!\.)'], ['.', r'…'],
            [' - ', r' -(?=[^\W\d_])'], [' - ', r'–'], [' ', r'[/\\|\[\]\{\}`]'], [' . ', r'=='],
            [' ', r'='], [' ', r'Categoria:\w+'], [' ', r'\*'], [r'\1', r'([,";:.]){,5}'],
            [' ', r' +'], [r'\1', r'\s+([,";:.])'], ['', r'\.(\s\w+\s?[-,:]?\s?){1,4}\.']
        ]
    ...
    def bigramas(self):
        ...
        probabilidades = {
            palavra: {proxima: count / unigramas[palavra] for proxima, count in seguinte.items()}
    ...
    def processar_frase(self, frase):
        ...
        for i in range(len(palavras) - 1):
            try:
                if self.gerar_texto(raiz=palavras[i], minimo=1, maximo=2).split()[-1] == palavras[i + 1]:
                    a += 1
                else:
                    e += 1
            except IndexError:
    ...
    def testar_modelo(self, tarefas=1):
        ...
        resultados = process_map(self.processar_frase, self.teste, max_workers=tarefas, chunksize=1000)
